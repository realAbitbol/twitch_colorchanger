"""Color change orchestration logic extracted from `bot.Bot`.

Handles:
 - Color selection (hex vs preset)
 - Performing API call with rate limiter
 - Single retry on 401 with token refresh
 - Fallback from random hex to preset (for Prime/Turbo users)

Public entrypoint: `await ColorChangeService.change_color(hex_color=None)` -> bool
"""
...
from .retry_policies import COLOR_CHANGE_RETRY, run_with_retry
...
class ColorChangeService:
    ...
    async def change_color(self, hex_color: str | None = None) -> bool:
        ...
        If hex_color provided, use it directly (no preset fallback).
        Otherwise select according to user settings (random hex for Prime/Turbo or preset).
        """
        ...
        if hex_color:
            ...
            allow_fallback = (
                self.bot.use_random_colors
            )  # only random hex should fallback
