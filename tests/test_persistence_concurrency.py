...
async def test_queue_vs_direct_last_wins(monkeypatch, tmp_path: Path):
    ...
    await queue_user_update({"username": "zoe", "channels": ["zoe", "x"]}, str(cfg))
    await queue_user_update({"username": "zoe", "channels": ["zoe", "x", "y"]}, str(cfg))
    # Direct write should be serialized with per-user lock (channels -> only zoe + y + z)
    await async_update_user_in_config({"username": "zoe", "channels": ["zoe", "z"]}, str(cfg))
    # Another queued update (should overwrite prior queued state before flush)
    await queue_user_update({"username": "zoe", "channels": ["zoe", "final"]}, str(cfg))
