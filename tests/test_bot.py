...
def test_handle_irc_message_self_increments(bot_config):
    ...
    with patch("asyncio.get_event_loop") as mock_get_loop, patch.object(
        asyncio, "run_coroutine_threadsafe"
    ) as run_coro, patch.object(bot, "_change_color", new_callable=AsyncMock):
...
async def test_start_success(
    ...
    with patch.object(bot, "_get_user_info", side_effect=mock_get_user_info):
        with patch.object(
            bot, "_get_current_color", side_effect=mock_get_current_color
        ):
            with patch.object(
                bot,
                "_check_and_refresh_token",
                side_effect=mock_check_and_refresh_token,
            ):
                with patch.object(
                    bot, "_periodic_token_check", side_effect=mock_periodic_token_check
                ):
...
async def test_start_no_user_id(mock_print_log, bot_config):
    ...
    with patch.object(bot, "_get_user_info", return_value=AsyncMock(return_value=None)):
        with patch.object(
            bot, "_check_and_refresh_token", return_value=AsyncMock(return_value=True)
        ):
            await bot.start()
...
def test_channel_deduplication_persistence(bot_config, tmp_path):
    ...
    unique_channels = list(
        dict.fromkeys([ch.lower().replace("#", "") for ch in bot.channels])
    )
...
def test_handle_irc_message_runtime_error_fallback(bot_config):
    ...
    with patch(
        "src.bot.asyncio.get_event_loop", side_effect=RuntimeError("No event loop")
    ):
...
async def test_check_and_refresh_token_no_expiry_validate_api(bot_config):
    ...
    with patch.object(
        bot, "_validate_token_via_api", return_value=True
    ) as mock_validate:
        result = await bot._check_and_refresh_token()
...
async def test_check_and_refresh_token_validation_failed(bot_config):
    ...
    with patch.object(bot, "_validate_token_via_api", return_value=False):
        with patch.object(
            bot, "_attempt_standard_refresh", return_value=True
        ) as mock_attempt:
            result = await bot._check_and_refresh_token()
...
async def test_validate_token_via_api_success(bot_config):
    ...
    bot = TwitchColorBot(**bot_config)
    with patch.object(
        bot, "_get_user_info", return_value=AsyncMock(return_value={"id": "12345"})
    ):
        with patch("src.bot.print_log") as mock_print_log:
...
def test_persist_token_changes_failure(bot_config):
    ...
    with patch(
        "src.bot.update_user_in_config", side_effect=RuntimeError("Save failed")
    ):
        with patch("src.bot.print_log") as mock_print_log:
            bot._persist_token_changes()
...
async def test_make_api_request_content_type_error(mock_session_class):
    ...
    mock_response.status = 200
    mock_response.json = AsyncMock(
        side_effect=aiohttp.ContentTypeError(None, None, message="Invalid content type")
    )
    mock_response.headers = {"Content-Type": "text/html"}
    ...
    with patch.object(bot, "_get_user_info", side_effect=mock_get_user_info):
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    bot, "_periodic_token_check", return_value=AsyncMock()
                ):
                    with patch("src.bot.asyncio.gather", side_effect=mock_gather):
                        with patch.object(
                            bot, "stop", return_value=AsyncMock()
                        ) as mock_stop:
                            await bot.start()
...
async def test_change_color_api_error_handling(bot_config, mock_rate_limiter):
    ...
    with patch.object(
        bot, "_attempt_color_change", side_effect=mock_attempt_color_change
    ):
...
async def test_try_preset_color_fallback_exception(bot_config, mock_rate_limiter):
    ...
    with patch(
        "src.bot.get_different_twitch_color", side_effect=RuntimeError("Test error")
    ):
        with patch("src.bot.logger") as mock_logger:
            result = await bot._try_preset_color_fallback()
...
class TestBotBranchCoverage:
    """Test missing branch coverage in bot.py"""

    @pytest.mark.asyncio
    async def test_start_user_info_missing_id_field(self):
        """Test branch when user_info doesn't contain 'id' field - lines 115->132"""
        bot = TwitchColorBot(
        ...
        with patch.object(
            bot, "_check_and_refresh_token", return_value=None
        ), patch.object(
            bot,
            "_get_user_info",
            return_value={"login": "testuser", "display_name": "TestUser"},
        ), patch(
            "src.bot.print_log"
        ) as mock_print:
            await bot.start()
            mock_print.assert_any_call(
                "❌ testuser: Failed to retrieve user_id", unittest.mock.ANY
            )

    @pytest.mark.asyncio
    async def test_start_user_info_none_response(self):
        """Test branch when user_info is None - lines 115->132"""
        bot = TwitchColorBot(
            token="oauth:token123",
            refresh_token="refresh123",
            client_id="client123",
        ...
        with patch.object(
            bot, "_check_and_refresh_token", return_value=None
        ), patch.object(bot, "_get_user_info", return_value=None), patch(
            "src.bot.print_log"
        ) as mock_print:
            await bot.start()
            mock_print.assert_any_call(
    ...
    @pytest.mark.asyncio
    async def test_start_get_current_color_none_response(self):
        """Test branch when current_color is None - lines 133->141"""
        bot = TwitchColorBot(
            token="oauth:token123",
            refresh_token="refresh123",
            client_id="client123",
        ...
        with patch.object(
            bot, "_check_and_refresh_token", return_value=None
        ), patch.object(bot, "_get_current_color", return_value=None), patch(
            "src.bot.SimpleTwitchIRC"
        ) as mock_irc_class, patch(
            "asyncio.create_task"
    ...
    @pytest.mark.asyncio
    async def test_refresh_access_token_impl_missing_refresh_token_in_response(self):
        """Test branch when refresh_token is not in response - lines 697->701"""
        bot = TwitchColorBot(
            token="oauth:token123",
            refresh_token="refresh123",
            client_id="client123",
        ...
        with patch("aiohttp.ClientSession") as mock_session_cls:
            ...
            assert bot.refresh_token == "refresh123"

    @pytest.mark.asyncio
    async def test_refresh_access_token_impl_missing_expires_in_response(self):
        """Test branch when expires_in is not in response - lines 701->709"""
        bot = TwitchColorBot(
            token="oauth:token123",
            refresh_token="refresh123",
            client_id="client123",
...
class TestRemainingBotCoverage:
    ...
    async def test_start_no_channel_dedup_else_branch(self):
        ...
        with patch("src.bot.SimpleTwitchIRC") as mock_irc_cls, patch(
            "src.bot.asyncio.get_event_loop"
        ) as mock_loop_fn:
            mock_irc = MagicMock()
            mock_irc.listen = MagicMock()
    ...
    async def test_persist_channel_deduplication_success_logging(self, tmp_path):
        ...
        with patch("src.bot.update_user_in_config") as mock_update, patch(
            "src.bot.print_log"
        ) as mock_log:
            bot._persist_channel_deduplication()
        ...
        success_calls = [
            ...
            if "Deduplicated channels saved to configuration" in c.args[0]
        ]
    ...
    async def test_start_channel_deduplication_branch_with_persistence(self, tmp_path):
        ...
        bot = TwitchColorBot(
            ...
            client_secret="csecret",
            nick="testuser",
            channels=["ChannelOne", "channelone", "#ChannelTwo", "channelone"],
            config_file=str(cfg_path),
        )
        bot.user_id = "12345"
        bot.access_token = "oauth:tok"
        ...
        bot.handle_irc_message = AsyncMock()  # type: ignore
        with patch("src.bot.SimpleTwitchIRC") as mock_irc_cls, patch(
            "src.bot.update_user_in_config"
        ) as mock_update, patch("src.bot.print_log") as mock_log, patch(
            "src.bot.asyncio.create_task", side_effect=RuntimeError("stop_after_dedup")
        ):
            dummy_irc = MagicMock()
        ...
        assert any(
            "Deduplicated channels" in c.args[0] for c in mock_log.call_args_list
        )
    ...
    def test_persist_channel_deduplication_failure_logging(self, tmp_path):
        ...
        bot.username = "testuser"
        with patch(
            "src.bot.update_user_in_config", side_effect=RuntimeError("fail")
        ), patch("src.bot.print_log") as mock_log:
            bot._persist_channel_deduplication()
        assert any(
            "Failed to save deduplicated channels" in c.args[0]
            for c in mock_log.call_args_list
        )
    ...
    def test_persist_channel_deduplication_no_config_file_branch(self):
        ...
        assert not any(
            "Deduplicated channels saved" in c.args[0] for c in mock_log.call_args_list
        )
        assert not any(
            "Failed to save deduplicated channels" in c.args[0]
            for c in mock_log.call_args_list
