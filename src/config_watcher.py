...
class ConfigFileHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        if os.path.abspath(event.src_path) == self.config_file:
            ...
            threading.Thread(target=self.watcher._on_config_changed, daemon=True).start()
...
class ConfigWatcher:
    ...
    def _on_config_changed(self):
        ...
        try:
            ...
            logger.info(f"✅ Config validation passed - {len(valid_users)} valid user(s)")
...
async def create_config_watcher(config_file: str, restart_callback: Callable) -> ConfigWatcher:
