...
class ScheduledTask:
    ...
    __slots__ = ['next_run', 'callback', 'name', 'interval', 'args', 'kwargs', 'priority']
...
class AdaptiveScheduler:
    ...
    async def schedule_recurring(
        ...
        **kwargs
        ...
        print_log(f"📅 Scheduled recurring task '{name}' (interval: {interval}s)", BColors.OKCYAN)
        return True
    ...
    async def schedule_once(
        ...
        **kwargs
        ...
        print_log(f"📅 Scheduled one-time task '{name}' (delay: {delay}s)", BColors.OKCYAN)
        return True
        ...
        async with self._lock:
            for task in self.tasks:
                if task.name == name and task.interval is not None:
                    ...
                    heapq.heapify(self.tasks)  # Rebuild heap
                    print_log(f"🔄 Rescheduled task '{name}' (new interval: {new_interval}s)", BColors.OKCYAN)
                    return True
    ...
    async def _execute_task(self, task: ScheduledTask):
        ...
        try:
            ...
            print_log(f"🚫 Task '{task.name}' cancelled", BColors.WARNING, debug_only=True)
            
        except Exception as e:
            print_log(f"❌ Task '{task.name}' failed: {e}", BColors.FAIL)
            logger.error(f"Task execution error", task_name=task.name, error=str(e))
