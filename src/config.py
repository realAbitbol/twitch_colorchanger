...
def print_config_summary(users):
    ...
    for i, user in enumerate(users, 1):
        ...
        print_log(f"   Random Colors: {'Yes' if user['use_random_colors'] else 'No'}")
        print_log(f"   Has Refresh Token: {'Yes' if user['refresh_token'] else 'No'}")
        print_log(f"   Has Client Credentials: {'Yes' if user['client_id'] and user['client_secret'] else 'No'}")
