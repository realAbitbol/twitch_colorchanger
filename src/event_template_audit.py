...
Scans the source tree for usages of logger.log_event(domain, action, ...)
and compares them to the entries loaded from event_catalog.EVENT_TEMPLATES
(which are backed by the JSON file `event_templates.json`).

Usage (CLI):
  python -m event_template_audit [options]

Or after installation (console script):
  event-templates-audit

Exit codes:
  0 - No missing templates
  1 - Missing templates detected
  2 - Other error
"""
...
from event_catalog import EVENT_TEMPLATES, reload_event_templates

PROJECT_ROOT = Path(__file__).resolve().parent
SRC_ROOT = PROJECT_ROOT  # This file already lives in src
TEMPLATES_JSON = PROJECT_ROOT / "event_templates.json"

# Regex patterns for positional and keyword usage of logger.log_event
...
def iter_python_files(root: Path) -> Iterable[Path]:
    for path in root.rglob("*.py"):
        if path.name.startswith("."):
            continue
        if path.name in {"event_catalog.py"}:  # avoid scanning template file itself
            pass  # still can scan; leave in for completeness
        yield path
...
def extract_references(paths: Iterable[Path]) -> set[tuple[str, str]]:
    ...
    for path in paths:
        ...
        except (OSError, UnicodeDecodeError):  # narrow exception for Bandit B112
            continue
...
def compute_diff() -> tuple[
    ...
    reload_event_templates()  # ensure fresh
    ...
    # Missing: referenced in code but not in JSON (and thus not in loaded templates)
...
def emit_human(d: DiffResult) -> None:
    ...
    print("============================")
    # Recompute counts for transparency (cheap operations)
    print(
