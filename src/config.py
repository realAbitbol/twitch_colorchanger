...
from .config.model import UserConfig, normalize_user_list
...
async def setup_missing_tokens(users, config_file):
    """Provision tokens for users missing access/refresh tokens.

    Uses new TokenProvisioner API returning (access, refresh, expiry). Updates user
    dicts in-memory and persists if any changes.
    """
    import aiohttp
